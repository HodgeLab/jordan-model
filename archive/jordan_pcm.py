...
def wind_week(cap, mean_cf):
    base = mean_cf + 0.12 * np.sin(
        np.linspace(0, 7 * 2 * np.pi, 168) + 1
    )  # diurnal-ish, higher at night
...
loadA = week(WIN, 3205)
solarA = solar_week(SOLAR_CAP, 0.42, 7, 17)
windA = wind_week(WIND_CAP, 0.32)
# WEEK B: min net load (spring) - mild demand peak ~2450, high solar CF
loadB = week(SPR, 2450, 0.88)
solarB = solar_week(SOLAR_CAP, 0.68, 6, 18)
windB = wind_week(WIND_CAP, 0.40)
...
def run_week(load, solar, wind, label):
    H = len(load)
    merit = sorted(units, key=lambda u: u["cost"])
    on = {u["name"]: False for u in units}
    ontime = {u["name"]: 0 for u in units}
    offtime = {u["name"]: 99 for u in units}
    tot_fuel = 0.0
    tot_start = 0.0
    starts = {u["name"]: 0 for u in units}
    energy = {u["name"]: 0.0 for u in units}
    curt = 0.0
    imp = 0.0
    netl = []
    margprice = []
    for h in range(H):
        ...
        for u in sorted(units, key=lambda x: -x["cost"]):
            if not on[u["name"]]:
                continue
            if ontime[u["name"]] < u["minup"]:
                continue
            if (
                cap_on - u["pmax"] >= need
                and sum(
                    x["pmin"] for x in units if on[x["name"]] and x["name"] != u["name"]
                )
                <= nl
            ):
                on[u["name"]] = False
        ...
        if minsum > nl:
            ...
            nl = load[h] - (re - cc)
            if minsum > nl:
                nl = minsum  # remainder would be export; clamp
        # dispatch
        rem = nl
        disp = {}
        for u in committed:
            ...
            rem -= u["pmin"]
        for u in committed:
            add = min(u["pmax"] - u["pmin"], max(0, rem))
            disp[u["name"]] += add
            rem -= add
        ...
        if rem > 1e-6:
            ...
            imp += i
            rem -= i
            tot_fuel += i * EGYPT_COST
            mp = EGYPT_COST
        margprice.append(mp)
        for u in committed:
            energy[u["name"]] += disp[u["name"]]
            tot_fuel += disp[u["name"]] * u["cost"]
        for u in units:
            if on[u["name"]]:
                ontime[u["name"]] += 1
                offtime[u["name"]] = 0
    ...
    print("\n===== %s =====" % label)
    print(
        "Demand: peak %.0f / min %.0f MW | Net load: peak %.0f / min %.0f MW"
        % (load.max(), load.min(), netl.max(), netl.min())
    )
    print(
        "Weekly energy: demand %.0f MWh | renewables used %.0f MWh | curtailed %.0f MWh (%.1f%%)"
        % (load.sum(), re_energy, curt, 100 * curt / (solar.sum() + wind.sum() + 1e-9))
    )
    print(
        "Total cost: fuel %.0f + startup %.0f = %.0f USD (avg %.1f USD/MWh)"
        % (
            tot_fuel,
            tot_start,
            tot_fuel + tot_start,
            (tot_fuel + tot_start) / load.sum(),
        )
    )
    print(
        "Egypt import: %.0f MWh | max net-load ramp: %.0f MW/h"
        % (imp, np.abs(np.diff(netl)).max())
    )
    print("%-20s %8s %7s %6s" % ("Plant", "Energy MWh", "CF %", "starts"))
    for u in merit:
        e = energy[u["name"]]
        cf = 100 * e / (u["pmax"] * H)
        if e > 1 or starts[u["name"]] > 0:
            print("%-20s %8.0f %6.1f %6d" % (u["name"], e, cf, starts[u["name"]]))
...
with open("jordan_pcm_profiles.csv", "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["week", "hour", "load_MW", "solar_MW", "wind_MW", "netload_MW"])
...
with open("jordan_uc_parameters.csv", "w", newline="") as f:
    ...
    for n, v in UC.items():
        w.writerow([n, v[0], v[1], v[2], round(v[0] * v[3]), v[4], v[5], v[6]])
